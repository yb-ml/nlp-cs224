...
def naiveSoftmaxLossAndGradient(
    ...
    word_vec_len = outsideVectors.shape[1]
    y_hat = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(y_hat[outsideWordIdx])

    delta = y_hat
    delta[outsideWordIdx] -= 1  # y_hat - y
    gradCenterVec = np.dot(np.transpose(outsideVectors, axes=(1,0)), delta.reshape(num_words, 1))
    gradOutsideVecs = np.dot(delta.reshape(num_words, 1), centerWordVec.reshape(1, word_vec_len))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        cur_loss, cur_center_grads, cur_outside_grads = word2vecLossAndGradient(
            centerWordVectors[center_index],
            word2Ind[outsideWord],
            outsideVectors,
            dataset
        )
        loss += cur_loss
        gradCenterVecs[center_index] += cur_center_grads.reshape(-1)
        gradOutsideVectors += cur_outside_grads
    ### END YOUR CODE
